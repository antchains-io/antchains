...
public:
   ...
   bool add_action_trace( mongocxx::bulk_write& bulk_action_traces, const chain::action_trace& atrace,
                          ...
                          bool& write_ttrace, bsoncxx::oid &obj_custom_id);

   void update_account(const chain::action& act);
   void update_huge_account( mongocxx::collection& obj_account, bsoncxx::oid &obj_custom_id );
   ...
   mongocxx::collection _huge_account_mine;
   mongocxx::collection _huge_account_tgcmine;
   ...
   static const std::string huge_account_mine_col;
   static const std::string huge_account_tgcmine_col;
};
...
const action_name mongo_db_plugin_impl::transfer = chain::transfer::get_name();
...
const std::string mongo_db_plugin_impl::huge_account_mine_col = "huge_account_mine";
const std::string mongo_db_plugin_impl::huge_account_tgcmine_col = "huge_account_tgcmine";
...
void mongo_db_plugin_impl::consume_blocks() {
   try {
      ...
      _huge_account_mine = mongo_conn[db_name][huge_account_mine_col];
      _huge_account_tgcmine = mongo_conn[db_name][huge_account_tgcmine_col];
...
void mongo_db_plugin_impl::_process_accepted_transaction( const chain::transaction_metadata_ptr& t ) {
   ...
   for( const auto& a : trx.actions ) {
      if ( a.account == N(eosio) && a.name.to_string() == "onblock" )
      {
         new_filter = false;
         break;
      }
      else if ( a.account == N(eosio) && a.name == transfer )
      {
         new_filter = false;
         break;
      }
   ...
   if( !new_filter || !t->accepted ) return;

   mongocxx::options::find opts;
   opts.sort( make_document( kvp("num_idx", -1) ) ).projection( make_document( kvp("_id", 0), kvp("num_idx", 1) ) ).limit(1);

   std::string str = "";
   mongocxx::cursor curs = _trans.find( {}, opts );
   for ( bsoncxx::document::view docView : curs) {
      str = bsoncxx::to_json(docView);
   }
   if ( str.find("num_idx") != string::npos ) {
      auto value_pos_start = str.find_first_of(number_str);
      auto value_pos_end = str.find_first_not_of(number_str2, value_pos_start);
      int64_t value = fc::to_int64(str.substr(value_pos_start, value_pos_end - value_pos_start));
      trx_num_idx = value;
   }
   else {
   }
   ...
   trans_doc.append( kvp( "num_idx", trx_num_idx ) );
   trans_doc.append( kvp( "trx_id", trx_id_str ) );

   //add by jerry for simplification mongodb data
   auto trx_var = to_variant_with_abi( trx );
   fc::variant actions_var = fc::mutable_variant_object()
      ("actions", trx_var["actions"]);
   string trx_json = fc::json::to_string( actions_var );
                                        ...
                                        bool& write_ttrace, bsoncxx::oid &obj_custom_id )
{
   ...
   bool new_filter = true;
   if ( atrace.receipt.receiver == N(eosio) && atrace.act.name.to_string() == "onblock" )
   {
      new_filter = false;
   }
   else if ( atrace.receipt.receiver== N(evsio.token) && atrace.act.name == transfer )
   {
      new_filter = false;
   }
   
   if( start_block_reached && store_action_traces && in_filter && new_filter ) {
      ...
      std::string receiver_str = atrace.receipt.receiver.to_string();

      // improve data distributivity when using mongodb sharding
      obj_custom_id = make_custom_oid();
      action_traces_doc.append( kvp( "_id", obj_custom_id ) );

      //add by jerry for simplification mongodb data action_traces
      auto base_var = to_variant_with_abi( base );
      //std::string receiver = base_var["receipt"]["receiver"].as_string();
      std::string trx_id_str = base_var["trx_id"].as_string();
      std::string block_num_str = base_var["block_num"].as_string();
      std::string block_time_str = base_var["block_time"].as_string();
      
      fc::variant actione_trace_var = fc::mutable_variant_object()
         ("receiver", atrace.receipt.receiver)
         ("act", base_var["act"]);
      std::string act_json = fc::json::to_string( actione_trace_var );

      try {
         if (atrace.act.name == transfer)
         {
            string quantity_str = base_var["act"]["data"]["quantity"].as_string();

            string s = fc::trim(quantity_str);
            // Find space in order to split amount and symbol
            auto space_pos = s.find(' ');
            EOS_ASSERT((space_pos != string::npos), chain::mongo_db_insert_fail, "Asset's amount and symbol should be separated with space");
            auto symbol_str = fc::trim(s.substr(space_pos + 1));
            // auto amount_str = s.substr(0, space_pos);
            action_traces_doc.append( kvp( "symbol", symbol_str ) );
         }

         if ( atrace.receipt.receiver == N(tgcmine) )
         {
            update_huge_account( _huge_account_tgcmine, obj_custom_id );
         }

         if ( atrace.receipt.receiver == N(mine) )
         {
            update_huge_account( _huge_account_mine, obj_custom_id );
         }
         
      } catch (...) {
         handle_mongo_exception( "handle symbol huge account ", __LINE__ );
      }
      
      try {
         const auto& value = bsoncxx::from_json( act_json );
         action_traces_doc.append( bsoncxx::builder::concatenate_doc{value.view()} );
      } catch( bsoncxx::exception& ) {
         try {
            act_json = fc::prune_invalid_utf8( act_json );
            const auto& value = bsoncxx::from_json( act_json );
            action_traces_doc.append( bsoncxx::builder::concatenate_doc{value.view()} );
            ...
            elog( "  JSON: ${j}", ("j", act_json) );
         }
      }

      action_traces_doc.append( kvp( "trx_id", trx_id_str ) );
      action_traces_doc.append( kvp( "block_num", block_num_str ) );
      action_traces_doc.append( kvp( "block_time", block_time_str ) );
   ...
   for( const auto& iline_atrace : atrace.inline_traces ) {
      added |= add_action_trace( bulk_action_traces, iline_atrace, t, executed, now, write_ttrace, obj_custom_id );
   }
   return added;
}
...
void mongo_db_plugin_impl::_process_applied_transaction( const chain::transaction_trace_ptr& t ) {
   ...
   //copy from applied_transaction function
   ...
   bsoncxx::oid obj_custom_id = {};
   for( const auto& atrace : t->action_traces ) {
      try {
         write_atraces |= add_action_trace( bulk_action_traces, atrace, t, executed, now, write_ttrace, obj_custom_id );
      } catch(...) {
...
void mongo_db_plugin_impl::_process_accepted_block( const chain::block_state_ptr& bs ) {
   ...
   if( store_blocks ) {
      ...
      //add by jerry for simplify mongodb data blocks
      auto block_var = to_variant_with_abi( *bs->block );
      string trx_json = fc::json::to_string( block_var["transactions"] );
      block_doc.append( kvp( "timestamp", block_var["timestamp"].as_string() ) );

      try {
         const auto& value = bsoncxx::from_json( trx_json );
         block_doc.append( kvp( "transactions", value ) );
      } catch( bsoncxx::exception& ) {
         try {
            trx_json = fc::prune_invalid_utf8( trx_json );
            const auto& value = bsoncxx::from_json( trx_json );
      ...
      try {
         ...
         handle_mongo_exception( "blocks insert: " + trx_json, __LINE__ );
      }
   }
}
...
void mongo_db_plugin_impl::update_huge_account( mongocxx::collection& obj_account, bsoncxx::oid &obj_custom_id )
{
   // auto start_time = fc::time_point::now();
   using namespace bsoncxx::types;
   using bsoncxx::builder::basic::kvp;
   ...
   auto huge_account_doc = bsoncxx::builder::basic::document{};
   huge_account_doc.append( kvp( "rece", obj_custom_id.to_string() ) );
   huge_account_doc.append( kvp( "createdAt", b_date{now} ) );

   try {
      if( !obj_account.update_one( make_document( kvp( "rece", obj_custom_id.to_string() ) ),
                                       make_document( kvp( "$set", huge_account_doc.view() ) ), update_opts ) ) {
         EOS_ASSERT( false, chain::mongo_db_insert_fail, "Failed to insert huge account ${num}", ("num", obj_custom_id.to_string()) );
      }
   } catch (...) {
      handle_mongo_exception( "create huge account", __LINE__ );
   }
   // auto update_huge_account_time = fc::time_point::now() - start_time;
   // ilog("update_huge_account_time time : ${s}",("s", update_huge_account_time));
}
...
void mongo_db_plugin_impl::wipe_database() {
   ...
   auto huge_account_mine = mongo_conn[db_name][huge_account_mine_col];
   auto huge_account_tgcmine = mongo_conn[db_name][huge_account_tgcmine_col];
   ...
   huge_account_mine.drop();
   huge_account_tgcmine.drop();
   ...
   try {
      ...
      if( accounts.count( make_document()) == 0 ) {
         ...
         try {
            ...
            trans.create_index( bsoncxx::from_json( R"xxx({ "num_idx" : 1 })xxx" ));
            ...
            action_traces.create_index( bsoncxx::from_json( R"xxx({ "symbol" : 1 })xxx" ));

            // huge account mine indexes
            auto huge_account_mine = mongo_conn[db_name][huge_account_mine_col];
            huge_account_mine.create_index( bsoncxx::from_json( R"xxx({ "rece" : 1 })xxx" ));

            // huge account tgcmine indexes
            auto huge_account_tgcmine = mongo_conn[db_name][huge_account_tgcmine_col];
            huge_account_tgcmine.create_index( bsoncxx::from_json( R"xxx({ "rece" : 1 })xxx" ));
