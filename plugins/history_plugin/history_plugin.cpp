      ...
      public:
         ...
         fc::optional<scoped_connection> applied_transaction_connection;
          ...
          bool filter(const action_trace& act) {
            ...
            /*
                 获取data中的数据，先要判断是否为转账交易。如果接收方是iketodeposit，则不接受写入数据库中。
            */
           ilog("filter act.name =${name}",("name",act.act.name.to_string()));
           if("transfer"!=act.act.name.to_string())
           {
               return false;
           }
           {
              ...
              auto act_var=chain.to_variant_with_abi(act, abi_serializer_max_time);
              if("iketodeposit"!=act_var["act"]["data"]["to"].as_string())
           }
