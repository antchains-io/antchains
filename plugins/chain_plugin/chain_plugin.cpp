...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         ("read-mode", boost::program_options::value<eosio::chain::db_read_mode>()->default_value(eosio::chain::db_read_mode::SPECULATIVE),
          ...
          ("safe-hard-replay-height", bpo::value<uint32_t>()->default_value(0), "modify eosio code then safe hard replay, set safety height skip condition." )
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      ...
      //add by jerry for safe hard replay blockchain when modify eosio code while upset
      my->chain_config->safe_replay_height = options.at("safe-hard-replay-height").as<uint32_t>();
...
/*
   make 中的参数api的含义如下：
   api->db.db(),需要弄清楚，作为数据源来查找name对应的abi
   最后根据account name对应的abi来实现数据的序列化

   切记看代码一知半解 yfl 20191022
*/
...
void read_write::push_transaction_v2(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   result.bind_check       = a.bind_check;
   result.extern_value     = a.extern_value;
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      const auto token_code = N(evsio.token);
