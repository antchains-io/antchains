...
authority parse_json_authority_or_key(const std::string& authorityJsonOrFile) {
   if (boost::istarts_with(authorityJsonOrFile, "EVS") || boost::istarts_with(authorityJsonOrFile, "PUB_R1")) {
      try {
...
inline asset to_asset( const string& s ) {
   return to_asset( N(evsio.token), s );
}
