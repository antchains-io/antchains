...
struct reflector_init_visitor {
 ...
 private:
   ...
   //判断fc::reflect_init是否为T的基类 yfl added 20191023
   //typename std::enable_if<std::is_base_of<fc::reflect_init, T>::value>::type为函数返回类型
   template<class T>
   ...
   //判断fc::reflect_init是否为T的基类 yfl added 20191023
   //typename std::enable_if<not std::is_base_of<fc::reflect_init, T>::value>::type为函数返回类型
   template<class T>
...
/*
    error:type对应的含义是什么，没有找到对应的含义
    BOOST_PP_STRINGIZE 对应的功能是转化成字符串

    通过重载运算符()来进行完成visit函数功能
*/
...
/*
    这里需要按照boost_pp_seq_for_each来展开相应的定义
    visit_base解释如下：
    根据FC_REFLECT_VISIT_BASE 宏定义，v为data，INHERITS为seq

    扩展后为FC_REFLECT_VISIT_MEMBER(r,visitor,elem).
    根据INHERITS来集合来完成多个FC_REFLECT_VISIT_MEMBER的声明
*/
