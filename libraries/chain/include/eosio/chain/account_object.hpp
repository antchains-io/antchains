...
namespace eosio { namespace chain {
   ...
   class account_object : public chainbase::object<account_object_type, account_object> {
      ...
      /* bind_check 类型涵义：
            0: 没有实名认证的用户，接受订单返币，但不能自己转出。
            1：实名认证的用户，接受订单转账返币，转账不受限制
            2：不接收订单返币，转账不受限制
      */
      uint8_t              bind_check    = 0; 
      uint64_t             extern_value = 0;  //Not used yet
...
FC_REFLECT(eosio::chain::account_object, (name)(vm_type)(vm_version)(privileged)(last_code_update)(code_version)(creation_date)(code)(abi)(bind_check)(extern_value))
