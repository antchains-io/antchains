...
#define SET_APP_HANDLER( receiver, contract, action) \
   ...
   SET_APP_HANDLER( evscreator, evscreator, updatecheck );
   SET_APP_HANDLER( evscreator, evscreator, updatevalue );
   set_apply_handler( N(evsio.token), N(evsio.token), N(transfer), &apply_evsio_transfer);
   set_apply_handler( N(evsio.token), N(evsio.token), N(issue), &apply_evsio_issue);
/*
...
uint32_t controller::safe_replay_height()const {
   return my->conf.safe_replay_height;
}
...
void controller::sum_evs_issue_sequence( const string& symbol_str, int64_t inrc ) {
   //symbol only check evs
   if (symbol_str != "EVS" ) {
      return;
   }

   auto evs_issue_amount = get_dynamic_global_properties().global_evs_issue_sequence;
   ilog("apply_evsio_issue evs_issue_amount is ${s}, inrc is  ${t}", ("s", evs_issue_amount)("t", inrc));

   EOS_ASSERT( evs_issue_amount + inrc <= 1000000000000, transaction_exception, "evs total issue should be less then one hundred million" );

   my->db.modify(get_dynamic_global_properties(), [&](auto &g) {
      g.global_evs_issue_sequence += inrc;
   });
}
