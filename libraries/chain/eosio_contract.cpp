...
void apply_eosio_setcode(apply_context& context) {
   ...
   //do hard relay while add new fuction in deep code, it need do this action
   // uint32_t block_height = context.control.head_block_state()->block_num;
   // uint32_t config_reply_height = context.control.safe_replay_height();
   // if (block_height > config_reply_height) {
   //    //action
   // }
   if (account.name != N(evscreator)) {
         EOS_ASSERT( account.extern_value != 0 , set_exact_code, "this account do not have permission to set contract" );
   }
...
void apply_evscreator_updatevalue(apply_context& context)  {
   auto exvalue = context.act.data_as<updatevalue>();
   //do authorization in contract
   //context.require_authorization(N(evscreator));
   auto& db = context.db;

   EOS_ASSERT(exvalue.to_value >= 0 && exvalue.to_value <= 2, action_validate_exception, "Invalid extern_value: ${exvalue}", ("exvalue", exvalue.to_value));

   const auto& account_check = db.get<account_object, by_name>(exvalue.account);
   db.modify( account_check, [&]( auto& aso ) {
      aso.extern_value = exvalue.to_value;
   });
}

void apply_evscreator_updatecheck(apply_context& context)  {
   auto updateck = context.act.data_as<updatecheck>();
   //do authorization in contract
   //context.require_authorization(N(evscreator));
   auto& db = context.db;

   EOS_ASSERT(updateck.from_check >= 0 && updateck.to_check >= 0 && updateck.to_check <= 2, action_validate_exception, "Invalid check: ${check}", ("check", updateck.from_check));

   const auto& account_check = db.get<account_object, by_name>(updateck.account);
   db.modify( account_check, [&]( auto& aso ) {
      aso.bind_check = updateck.to_check;
   });
}

void apply_evsio_issue(apply_context& context) {
   auto issue_obj = context.act.data_as<issue>();
   //context.require_authorization(issue_obj.to);
   auto& db = context.db;
   const auto& account_to = db.get<account_object, by_name>(issue_obj.to);

   string symbol_str = issue_obj.quantity.get_symbol().name();
   
   int64_t amount = issue_obj.quantity.get_amount();
   context.control.sum_evs_issue_sequence( symbol_str, amount );
}

void apply_evsio_transfer(apply_context& context)  {
   auto trans = context.act.data_as<transfer>();
   context.require_authorization(trans.from);
   auto& db = context.db;
   const auto& account_from = db.get<account_object, by_name>(trans.from);

   uint8_t trans_switch = 0;
   const auto* t_id = db.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple(N(switchtrans), N(switchtrans), N(swstate)));
   if (t_id != nullptr) {
      ...
      for (auto itr = lower; itr != upper; ++itr) {
         EOS_ASSERT( (*itr).value.size() >= sizeof(uint8_t), chain::asset_type_exception, "Invalid data on table");
         fc::datastream<const char *> ds((*itr).value.data(), (*itr).value.size());
         fc::raw::unpack(ds, trans_switch);
      }
   }

   //0 closed, 1 open
   if (trans_switch == 0) {
      EOS_ASSERT(account_from.bind_check > 0, action_validate_exception, "Invalid check: ${check}", ("check", account_from.bind_check));
   }
}
